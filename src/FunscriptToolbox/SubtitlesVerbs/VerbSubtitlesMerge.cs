...
{
    class VerbSubtitlesMerge : Verb
    {
        private static readonly ILog rs_log = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        [Verb("subtitles.merge", aliases: new[] { "sub.merge" }, HelpText = "Merge srts files")]
        public class Options : OptionsBase
        {
            ...
            [Option('p', "name pattern", HelpText = "Pattern to extract start and times from the name of the file", Default = @"^\d+_(?<StartHours>\d+)_(?<StartMinutes>\d+)_(?<StartSeconds>\d+)_(?<StartMilliseconds>\d+)__(?<EndHours>\d+)_(?<EndMinutes>\d+)_(?<EndSeconds>\d+)_(?<EndMilliseconds>\d{3})")]
        ...
        public VerbSubtitlesMerge(Options options)
            : base(rs_log, options)
        {
            r_options = options;
                ...
                if (string.Equals(Path.GetExtension(filepath), SubtitleFile.SrtExtension, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        if (inputSrt.Subtitles.Count == 0)
                        {
                            ...
                            outputFile.Subtitles.Add(new Subtitle(fileStartTime, fileEndTime, "** Nothing Detected **"));
                        }
                        else
                        {
